...
/******/ 	!function() {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "Weld Tech Group:";
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkWeld_Tech_Group"] = self["webpackChunkWeld_Tech_Group"] || [];
...
if (!fetchApi && !XmlHttpRequestApi && !ActiveXObjectApi) {
  try {
    __webpack_require__.e(/* import() */ 945).then(__webpack_require__.t.bind(__webpack_require__, 945, 19)).then(function (mod) {
...
const {
  slice: i18nextBrowserLanguageDetector_slice,
  forEach
} = [];
function i18nextBrowserLanguageDetector_defaults(obj) {
  forEach.call(i18nextBrowserLanguageDetector_slice.call(arguments, 1), source => {
    if (source) {
      for (const prop in source) {
        if (obj[prop] === undefined) obj[prop] = source[prop];
      }
...
class Browser {
  ...
  init() {
    ...
    this.services = services;
    this.options = i18nextBrowserLanguageDetector_defaults(options, this.options || {}, i18nextBrowserLanguageDetector_getDefaults());
...
instance.use(esm)
  ...
  .init({
    ...
    supportedLngs,
    debug: true,
    ns: ['common', 'header', 'home', 'about', 'services', 'projects', 'equipment', 'industry'],
    defaultNS: 'common',
    backend: {
      loadPath: './locales/{{lng}}/{{ns}}.json'
    },
...
/* harmony default export */ var i18n = ((/* unused pure expression or super */ null && (i18next))); 
...
function translateElements() {
  ...
  elements.forEach(element => {
    ...
    if (key) {
      ...
      element.textContent = instance.t(fullKey, { ns: namespace });
    }
  });
}
...
function translateAttributes() {
  ...
  elements.forEach(element => {
    ...
    Object.entries(attributes).forEach(([attr, key]) => {
      ...
      const [namespace, ...keyParts] = key.split('.');
      const fullKey = keyParts.join('.');
      element.setAttribute(attr, instance.t(fullKey, { ns: namespace }));
    });
  });
}
...
// Переводим элементы после инициализации i18next
...
var splide_esm_isString = apply(typeOf, "string");
...
function addClass(elm, classes) {
  toggleClass(elm, splide_esm_isString(classes) ? classes.split(" ") : classes, true);
}
...
function create(tag, attrs, parent) {
  ...
  if (attrs) {
    splide_esm_isString(attrs) ? addClass(elm, attrs) : setAttribute(elm, attrs);
  }
...
function Slides(Splide2, Components2, options) {
  ...
  function filter(matcher) {
    return Slides2.filter(isFunction(matcher) ? matcher : function (Slide2) {
      return splide_esm_isString(matcher) ? matches(Slide2.slide, matcher) : includes(toArray(matcher), Slide2.index);
    });
  }
...
function Drag(Splide2, Components2, options) {
  ...
  function mount() {
    bind(track, POINTER_MOVE_EVENTS, splide_esm_noop, SCROLL_LISTENER_OPTIONS);
    bind(track, POINTER_UP_EVENTS, splide_esm_noop, SCROLL_LISTENER_OPTIONS);
...
function Pagination(Splide2, Components2, options) {
  ...
  function destroy() {
    if (list) {
      remove(placeholder ? splide_esm_slice(list.children) : list);
...
var _Splide = /*#__PURE__*/function () {
  function _Splide(target, options) {
    ...
    this._o = {};
    this._E = {};
    var root = splide_esm_isString(target) ? query(document, target) : target;
...
jquery_default()(function() {
  if (document.querySelector('.splide')) {
    ...
    function checkVisibility() {
      jquery_default()('.animate-on-scroll').each(function () {
          let elem = jquery_default()(this);
          let elemTop = elem.offset().top;
          let windowBottom = jquery_default()(window).scrollTop() + jquery_default()(window).height();
  ...
  jquery_default()(window).on('scroll', checkVisibility);
  checkVisibility(); 
  ...
  function setEqualHeight(elements) {
    ...
    jquery_default()(elements).css('height', 'auto');

    jquery_default()(elements).each(function () {
      let elementHeight = jquery_default()(this).outerHeight();
  ...
  if(jquery_default()(window).width() > 767) {
    ...
    function updateLineHeight() {
      let row = jquery_default()('.work-stages-row');
      let firstCircle = jquery_default()('.work-stages-item.item-1 .stage-circle');
      let lastCircle = jquery_default()('.work-stages-item.item-6 .stage-circle');
      let line = jquery_default()('.stage-line');
  ...
  jquery_default()(".close, #modal-window").click(function(event){
  ...
  jquery_default()('#contact-form').on('submit', function (e) {
    e.preventDefault(); 
    ...
    let formData = jquery_default()(this).serialize();
    ...
    jquery_default().ajax({
        ...
        success: function (response) {
            jquery_default()('#form-status')
                .removeClass('error')
                .addClass('success')
                .text('Сообщение успешно отправлено!')
                .css("display", "inline-block");
            jquery_default()('#contact-form')[0].reset(); 
        },
        error: function () {
            jquery_default()('#form-status')
                .removeClass('success')
    ...
    const notification = jquery_default()('#cookie-notification');
    ...
    jquery_default()('#accept-cookies').on('click', function () {
